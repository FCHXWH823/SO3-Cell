...
if { [lsearch $operations "PNS"] >=0 } {
  ...
  if {[get_attribute -quiet [current_block] port_placement_export_file] != ""} {
    ...
    if {[sizeof_collection $port_list] > 0} {
      ...
      set origin "[expr [lindex [get_attribute [current_design] boundary] 0 0] - 50] [expr [lindex [get_attribute [current_design] boundary] 0 1] - 50]"
      move_objects -simple -to $origin $port_list
    }
  }

...
set DESIGN_NAME 		"aes_cipher_top" ;# Required; name of the design to be worked on; used as the block name for save_block or copy_block operations
...
set TECHLIB_DATA_DIR		"./data" ;# Used to point to directory where technology data is installed for the site. 
                                   ;# Used by variables which use data from those directories.

set SUPPLEMENTAL_SEARCH_PATH		"./data" ;# Optional; a list of paths; addtional user provided search_path which is to be used by set search_path in rm_setup/header_*.tcl 
...
set BLOCK_ABSTRACT_FOR_COMPILE          "$ICV_IN_DESIGN_BLOCK_NAME abstract" ;#  Define block label and view for compile.  The view can be "abstract" (default) or "design".
...
##########################################################################################
set NETLIST2GDS_FLOW		false ;# false|true; default false; optional in RM; 
				;# specify true if your design input is gated netlist (INIT_DESIGN_INPUT = ASCII) or write_icc2_files output from DC (INIT_DESIGN_INPUT = DC_ASCII)
set INIT_DESIGN_INPUT 		"RTL" ;# Specify one of the 4 options: RTL | ASCII | DC_ASCII | NDM; default is RTL; minimum required design inputs are listed below accordingly; 
 				;# 1.RTL: RTL netlist and other ascii design inputs files; 
				;#   required inputs: specify RTL_SOURCE_FORMAT with desired format below (and additional required variables as per RTL_SOURCE_FORMAT requirements), $UPF_FILE 
				;# 2.ASCII: gated netlist and other ascii design input files;
				...
				;# 3.DC_ASCII: for design transfer from DC using the write_icc2_files command; sources ${DCRM_RESULTS_DIR}/${DCRM_FINAL_DESIGN_ICC2}/${DESIGN_NAME}.icc2_script.tcl;
				...
				;#   note: init_design.tcl assumes floorplan is part of the write_icc2_files output and does not source any addtional floorplan fills  
			      	;# 4.NDM: specify your own floorplanned NDM path and skip the design creation steps (netlist reading, load_upf, read_def etc are skipped);
...
set RTL_SOURCE_FORMAT		sverilog ;# Specify one format out of verilog | sverilog | vhdl | script | elaborated_ndm
				;# - if RTL_SOURCE_FORMAT = verilog | sverilog | vhdl, init_design.tcl runs analyze -format $RTL_SOURCE_FORMAT $RTL_SOURCE_FILES; required inputs: $RTL_SOURCE_FILES; 
				;# - if RTL_SOURCE_FORMAT = script, init_design.tcl sources $FC_RTL_READ_SCRIPT; required inputs: $FC_RTL_READ_SCRIPT;
				;# - if RTL_SOURCE_FORMAT = elaborated_ndm, init_design.tcl copies $INIT_DESIGN_INPUT_LIBRARY and $INIT_DESIGN_INPUT_BLOCK_NAME as RTL source; 
				;#   required inputs: $INIT_DESIGN_INPUT_LIBRARY, and $INIT_DESIGN_INPUT_BLOCK_NAME 
...
set REFERENCE_LIBRARY 			[glob ./data/*.lef]	;# Required; a list of reference libraries for the design library.
...
set CLIB_REFERENCE_LIBRARY_CONFIGURATION_FLOW_FRAME_LIST [glob ./data/ndm/*.ndm] ;# a list of frames for on-the-fly CLIB reference library creation; ex : "[glob /path1/std/* /path2/*]"
set CLIB_REFERENCE_LIBRARY_CONFIGURATION_FLOW_DB_LIST [glob ./data/*.db] ;# a list of dbs for on-the-fly CLIB reference library creation; ex : "[glob /path1/std/db/*.db /path2/sram/db/*.db]"

## For on-the-fly fusion reference library creation using Makefile's create_fusion_reference_library target
set FUSION_REFERENCE_LIBRARY_FRAM_LIST  [glob ./data/ndm/*.ndm] ;# a list of frames (or ndm) for on-the-fly fusion reference library creation; ex : "[glob /path1/std/ndm/*.ndm /path2/sram/ndm/*.ndm]"
set FUSION_REFERENCE_LIBRARY_DB_LIST 	[glob ./data/*.db] ;# a list of dbs for on-the-fly fusion reference library creation; ex : "[glob /path1/std/db/*.db /path2/sram/db/*.db]"
...
set LINK_LIBRARY			"./data/*.db"	;# Optional; specify a list of .db files
...
##################################################
set TECH_FILE 			"./data/6T_BEOL1_24_30_24_24_64_64_64_64_64_64_64_64_720_720.tf" 	;# A technology file; TECH_FILE and TECH_LIB are mutually exclusive ways to specify technology information; 
...
##################################################
set TCL_MCMM_SETUP_FILE		"examples/TCL_MCMM_SETUP_FILE.explicit.tcl"	;# Specify a Tcl script to create your corners, modes, scenarios and load respective constraints;
...
set TCL_PARASITIC_SETUP_FILE	"examples/TCL_PARASITIC_SETUP_FILE.tcl"	;# Specify a Tcl script to read in your TLU+ files by using the read_parasitic_tech command;
...
set RTL_SOURCE_FILES		{./data/rtl/aes_cipher_top.v \
./data/rtl/aes_key_expand_128.v \
./data/rtl/aes_rcon.v \
./data/rtl/aes_sbox.v \
} ;# RTL files for RTL_SOURCE_FORMAT = verilog | sverilog | vhdl
set FC_RTL_READ_SCRIPT		${DESIGN_NAME}.FC.read_design.tcl ;# for RTL_SOURCE_FORMAT = script
...
##################################################
set TCL_FLOORPLAN_FILE			"./data/pdn_icc2.tcl" ;# Optional; Tcl floorplan file written by the write_floorplan command; for example, floorplan/floorplan.tcl;
...
set DFT_INSERT_ENABLE   true            ;# Use to enable sourcing of DFT_SETUP_FILE and performing insert_dft.
...
set TCL_USER_DFT_REPLACEMENT_SCRIPT	"" ;# User provided DFT insertion script.  When provided, the out of the box insert DFT flow is bipassed.
...
set TCL_USER_COMPILE_INITIAL_MAP_POST_SCRIPT "compile_initial_map_post_script.tcl" ;# An optional Tcl file for compile.tcl to be sourced after compile_fusion initial_map 
set TCL_USER_COMPILE_LOGIC_OPTO_PRE_SCRIPT "compile_logic_opto_pre_script.tcl" ;# An optional Tcl file for compile.tcl to be sourced before compile logic_opto
set TCL_USER_COMPILE_LOGIC_OPTO_POST_SCRIPT "compile_logic_opto_post_script.tcl" ;# An optional Tcl file for compile.tcl to be sourced after compile logic_opto
set TCL_USER_DFT_SETUP_PRE_SCRIPT	"dft_setup_pre_script.tcl" ;# An optional Tcl file for compile.tcl to be sourced before in-compile DFT insertion
set TCL_USER_DFT_POST_SCRIPT		"dft_post_script.tcl" ;# An optional Tcl file for compile.tcl to be sourced after in-compile DFT insertion
set TCL_USER_COMPILE_INITIAL_PLACE_PRE_SCRIPT "compile_initial_place_pre_script.tcl" ;# An optional Tcl file for compile.tcl to be sourced after compile_fusion logic_opto before initial_place 
set TCL_USER_COMPILE_INITIAL_DRC_PRE_SCRIPT "compile_initial_drc_pre_script.tcl" ;# An optional Tcl file for compile.tcl to be sourced after compile_fusion initial_place before initial_drc
set TCL_USER_COMPILE_INITIAL_OPTO_PRE_SCRIPT "compile_initial_opto_pre_script.tcl" ;# An optional Tcl file for compile.tcl to be sourced after compile_fusion initial_drc before initial_opto
set TCL_USER_COMPILE_INCREMENTAL_INITIAL_OPTO_PRE_SCRIPT "compile_incremental_initial_opto_pre_script.tcl" ;# An optional Tcl file for compile.tcl to be sourced before incremental initial_opto
set TCL_USER_COMPILE_FINAL_PLACE_PRE_SCRIPT "compile_final_place_pre_script.tcl" ;# An optional Tcl file for compile.tcl to be sourced after compile_fusion initial_opto before final_place
set TCL_USER_COMPILE_FINAL_OPTO_PRE_SCRIPT "compile_final_opto_pre_script.tcl" ;# An optional Tcl file for compile.tcl to be sourced after compile_fusion final_place before final_opto 
...
set REPORT_VERBOSE			true ;# true|false; RM default false; runs additional report_timing with -max_paths equal to 300 and -slack_lesser_than 0
...
set REPORT_COMPILE_ACTIVE_SCENARIO_LIST        "" ;# A subset of scenarios to be made active when generating reports for the compile step;
...
##########################################################################################
## HPC Core Variables specific to the HPC flow
##########################################################################################
set HPC_CORE                    "" ;# HPC core name; Example : A55, A76; enables set_hpc_options in the flow

set HPC_CORE_TOP 		"" ;# Hierarchical HPC flow only; Name of the top-level core.

## Below variables are used in flatten.tcl
set FLATTEN_BLOCK_NAME 			"flatten" ;# Label name to be used when saving a block in flatten.tcl
set TCL_USER_FLATTEN_PRE_SCRIPT 	"" ;# An optional Tcl file to be sourced at the beginning of flatten.tcl
set TCL_USER_FLATTEN_POST_SCRIPT 	"" ;# An optional Tcl file to be sourced at the end of flatten.tcl
set FLATTEN_ACTIVE_SCENARIO_LIST 	"" ;# A subset of scenarios to be made active during the flatten step
set TCL_USER_UNCOMMIT_POST_SCRIPT 	"" ;# An optional Tcl file to be sourced after uncommit
set TCL_USER_MCMM_SETUP_POST_SCRIPT 	"" ;# An optional Tcl file to be sourced after MCMM sourcing
set TCL_USER_UPF_SETUP_POST_SCRIPT 	"" ;# An optional Tcl file to be sourced after UPF sourcing

...
set SET_QOR_STRATEGY_METRIC		"timing" ;# timing|leakage_power|total_power; RM default is timing;
...
set SET_QOR_STRATEGY_MODE		"balanced" ;# balanced|extreme_power|early_design; RM default is balanced;
					;# Specify one mode for set_qor_strategy -mode option and the settings will be configured for the target mode
set SET_QOR_STRATEGY_CONGESTION_EFFORT	"" ;# ultra; RM default is unspecified (an empty string, i.e. "");
					;# Specify ultra to enable set_qor_strategy -congestion_effort ultra, which is intended for highly congested designs
...
set ENABLE_MULTIBIT                     true ;# false | true; RM default false but will be set to true if SET_QOR_STRATEGY_METRIC is set to total_power. 
...
set ENABLE_DPS				false ;# false|true; RM default false; set it to true for set_stage command to enable dynamic power shaping (DPS);
					;# only used by set_stage command which is part of the RM scripts and not used by RM scripts directly.
...
set INCR_ROUTE_DETAIL_MAX_ITERATIONS    "10" ;# (optional) a positive integer; if specified, add "-max_number_iterations $INCR_ROUTE_DETAIL_MAX_ITERATIONS" to the route_detail command;

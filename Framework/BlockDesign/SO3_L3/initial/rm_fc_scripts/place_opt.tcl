...
if {$DESIGN_STYLE == "hier"} {
	if {$USE_ABSTRACTS_FOR_BLOCKS != "" && ($BLOCK_ABSTRACT_FOR_PLACE_OPT != $BLOCK_ABSTRACT_FOR_COMPILE)} {
		puts "RM-info: Swapping from [lindex $BLOCK_ABSTRACT_FOR_COMPILE 0] to [lindex $BLOCK_ABSTRACT_FOR_PLACE_OPT 0] abstracts for all blocks."
		change_abstract -references $USE_ABSTRACTS_FOR_BLOCKS -label [lindex $BLOCK_ABSTRACT_FOR_PLACE_OPT 0] -view [lindex $BLOCK_ABSTRACT_FOR_PLACE_OPT 1]
		report_abstracts
	}
}

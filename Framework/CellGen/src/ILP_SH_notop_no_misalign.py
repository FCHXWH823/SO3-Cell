...
num_cols = 4 * max_trans + 3  # generous upper bound for mode3 guards
...
for c in gate_cols:
    ...
    # misalign_c alias: mode3 for this column (no mode4 in no_misalign variant)
    misalign_c[c] = mode3[c]
...
if model.status == GRB.OPTIMAL or ((model.status in (GRB.TIME_LIMIT, GRB.INTERRUPTED)) and model.SolCount > 0):
    ...
    if model.status == GRB.TIME_LIMIT:
        ...
        try:
            model.computeIIS()
            model.write("iis_no_misalign.ilp")
            pr("IIS written to iis_no_misalign.ilp")

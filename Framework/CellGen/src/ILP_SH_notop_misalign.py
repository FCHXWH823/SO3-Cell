...
num_cols = 4 * max_trans + 3  # generous upper bound for mode3/mode4 guards
...
# Pre-define mode4 variables (poly-cut columns: both rows real, different gate nets)
mode4 = {}
for c in gate_cols:
    mode4[c] = model.addVar(vtype=gp.GRB.BINARY, name=f"mode4_c{c}")
...
for c in range(num_cols):
    ...
    model.addConstr(gp.quicksum(nmos_net[(net, c)] for net in unique_nmos_nets) == 1,name=f"nmos_one_net_per_col_{c}")
...
col_used = {}
spacer  = {}
misalign_c = {}  # alias for downstream compatibility (flow_estimator, output)
for c in gate_cols:
    mode3[c]    = model.addVar(vtype=gp.GRB.BINARY, name=f"mode3_c{c}")
    col_used[c] = model.addVar(vtype=gp.GRB.BINARY, name=f"col_used_c{c}")
    spacer[c]   = model.addVar(vtype=gp.GRB.BINARY, name=f"spacer_c{c}")
...
for c in gate_cols:
    ...
    # Guard columns: adjacent gate cols must be fully dummy when mode3[c]=1
    ...
    # misalign_c alias: mode3 + mode4 for this column
    misalign_c[c] = mode3[c] + mode4[c]

# ---- CPP = rightmost used gate slot (1-indexed) ----
cpp_var = model.addVar(vtype=gp.GRB.INTEGER, lb=1, name="cpp")
...
b_c = {}
for c in gate_cols:
    a_c[c] = model.addVar(vtype=gp.GRB.INTEGER, lb=0, name=f"a_{net}_col{c}_final")
    b_c[c] = model.addVar(vtype=gp.GRB.INTEGER, lb=0, name=f"b_{net}_col{c}_final")
...
if model.status == GRB.OPTIMAL or ((model.status in (GRB.TIME_LIMIT, GRB.INTERRUPTED)) and model.SolCount > 0):
    if model.status == GRB.OPTIMAL:
        pr("Optimal solution found.")
    else:
        pr(f"Best feasible solution (status={model.status}, gap={model.MIPGap*100:.2f}%).")
    ...
    for c in gate_cols:
        _mc = misalign_c[c].X if hasattr(misalign_c[c], 'X') else misalign_c[c].getValue()
        print (f"Column : {c}, {a_c[c].X}, {b_c[c].X}, {_mc}, {e_c[c].getValue()}")
    ...
    if model.status == GRB.TIME_LIMIT:
        ...
        try:
            model.computeIIS()
            model.write("iis_misalign.ilp")
            pr("IIS written to iis_misalign.ilp")
            for con in model.getConstrs():
                if con.IISConstr:
                    pr(f"  IIS: {con.ConstrName}")
